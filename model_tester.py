...
#     # https://github.com/yigitcankaya/Shallow-Deep-Networks/blob/1719a34163d55ff237467c542db86b7e1f9d7628/model_funcs.py#L156
    ...
    def saver(self, experiment_id):
        ...
        with open("test_evaluation_log_"+str(experiment_id)+".txt", "w") as file:
            ...
            for layer in range(len(self.cur_correct_saver)):
                ...
                if self.exit_only:
                    print(f"E ({dropout_rate},{str(layer)}), {str(self.accu_saver[layer])}, {str(self.ece_saver[layer])}, ")
                    print(f"E ({dropout_rate},Ensemble{str(layer)}), {str(self.ensemble_accu_saver[layer])}, {str(self.ensemble_ece_saver[layer])}, ")
                elif self.model.dropout == "block":
                    print(f"B+E ({dropout_rate},{str(layer)}), {str(self.accu_saver[layer])}, {str(self.ece_saver[layer])}, ")
                    print(f"B+E ({dropout_rate},Ensemble{str(layer)}), {str(self.ensemble_accu_saver[layer])}, {str(self.ensemble_ece_saver[layer])}, ")
                elif self.model.dropout == "layer":  
                    print(f"L+E ({dropout_rate},{str(layer)}), {str(self.accu_saver[layer])}, {str(self.ece_saver[layer])}, ")
                    print(f"L+E ({dropout_rate},Ensemble{str(layer)}), {str(self.ensemble_accu_saver[layer])}, {str(self.ensemble_ece_saver[layer])}, ")                                  
    ...
    def get_confidence_exiting_values(self, model_num):
        ...
        for threshold in confidence_list:
            ...
            ensembled_accuracy, ensembled_ece = self.confidence_exiting(threshold, ensembled_p_evals, labels, diff = False)
            accuracy, ece = self.confidence_exiting(threshold, p_evals, labels, diff = False)
            if self.exit_only:
                print(f"E ({dropout_rate},{threshold}), {accuracy}, {ece}, ,{flops}")
                print(f"Ensemble E ({dropout_rate},{threshold}), {ensembled_accuracy}, {ensembled_ece}, ,{ensembled_flops}")
            elif self.model.dropout == "block":
                print(f"B+E ({dropout_rate},{threshold}), {accuracy}, {ece}, ,{flops}")
                print(f"Ensemble B+E ({dropout_rate},{threshold}), {ensembled_accuracy}, {ensembled_ece}, ,{ensembled_flops}")
            elif self.model.dropout == "layer":
                print(f"L+E ({dropout_rate},{threshold}), {accuracy}, {ece}, ,{flops}")
                print(f"Ensemble L+E ({dropout_rate},{threshold}), {ensembled_accuracy}, {ensembled_ece}, ,{ensembled_flops}")
        return None
    
    def get_flops_per_module(self):
        if self.model_type == "msdnet":
            self.n_exits = 11
            self.flops_per_layer = [8899584,9166848,16551936,7507968,10825728,14143488,25552896,10825728,14143488,17461248,9219072]
            self.flop_per_exit_convs = [2806784,3691520,2364416,3249152,4133888,5018624,3249152,4133888,5018624,5903360,6788096]
            self.flops_per_exit = [12800,12800,12800,12800,12800,12800,12800,12800,12800,12800,12800]
        elif self.model_type == "resnet20": # this is resnet 20 though right?
            self.n_exits = 10
            self.flops_per_layer = [475136+4784128,4784128,4784128,3571712,4751360,4751360,3555328,4734976,4734976]
            self.flops_per_exit = [26128,26128,26128,18224,18224,18224,15576,15576,15576,10496]
    ...
    def get_model_type(self):
        if isinstance(self.model,VGG):
            ...
            return "resnet18"
        elif isinstance(self.model,MsdNet):
            return "msdnet"
        else:
            raise ValueError
