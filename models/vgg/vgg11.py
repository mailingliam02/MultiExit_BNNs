...
def get_vgg_11(ensemble, network_hyperparams):
    if ensemble is None:
        # n_exits, out_dim
        model = VGG11(**dict_drop(network_hyperparams,"call", "load_model",
            "resnet_type","dropout","dropout_exit", "dropout_p"))

    elif ensemble == "early_exit":
        # n_exits, out_dim
        model = VGG11EarlyExit(**dict_drop(network_hyperparams,"call", "load_model",
            "resnet_type","dropout","dropout_exit", "dropout_p"))

    elif ensemble == "mc":
        # n_exits, out_dim, dropout, dropout_exit, dropout_p
        model = VGG11MC(**dict_drop(network_hyperparams,"call", "load_model",
            "resnet_type"))
     
    elif ensemble == "mc_early_exit":
        # n_exits, out_dim, dropout, dropout_exit, dropout_p
        model =  VGG11MCEarlyExit(**dict_drop(network_hyperparams,"call", "load_model",
            "resnet_type"))
    else:
        raise ValueError
...
def load_imagenet_weights(model):
    state_dict = torch.load("./MultiExit_BNNs/models/model_weights/vgg11_bn-c79401a0.pth")
...
def check_weight_load(model):
    if type(model) == VGG11 or model.dropout is None or model.dropout == "block":
...
class VGG(nn.Module):
    ...
    def __init__(self, blocks, num_class=10, image_size = 32):
        super().__init__()
        self.blocks, self.non_sequentialized_blocks = blocks
        self.image_size = image_size
        self.avg_pool = nn.AdaptiveAvgPool2d((7, 7))
        self.classifier = make_classifier(self.image_size,num_class)
        self.init_weights()
...
def make_classifier(size, num_classes, mc_dropout_p = 0):
    if size == 224:
        if mc_dropout_p == 0:
        ...
        if mc_dropout_p == 0:
            classifier =  nn.ModuleList([nn.Linear(128, num_classes)])
        else:
...
class VGG11MC(VGG11):
    def __init__(self,dropout_exit = False, dropout = None, dropout_p = 0.5, n_exits = 1, out_dim = 100, *args,  **kwargs):
        ...
        if self.dropout_exit:
            self.classifier = make_classifier(self.image_size,self.out_dim,self.dropout_p)
        # Load weights
        self.init_weights()
...
class VGG11EarlyExit(VGG11):
    def __init__(self, *args,  **kwargs):
        ...
        ex1conv1 = nn.Conv2d(16, 32, kernel_size=3, stride=2, padding=1, bias=False)
        ex1conv2 = nn.Conv2d(32, 64, kernel_size=3, stride=2, padding=1, bias=False)
        ex1conv3 = nn.Conv2d(64, 128, kernel_size=3, stride=2, padding=1, bias=False)
        ex1bn1 = nn.BatchNorm2d(32)
        ex1bn2 = nn.BatchNorm2d(64)
        ex1bn3 = nn.BatchNorm2d(128)
        ...
        self.ex1linear = make_classifier(self.image_size, self.out_dim) 
        ...
        ex2conv1 = nn.Conv2d(32, 64, kernel_size=3, stride=2, padding=1, bias=False)
        ex2conv2 = nn.Conv2d(64, 128, kernel_size=3, stride=2, padding=1, bias=False)
        ex2bn1 = nn.BatchNorm2d(64)
        ex2bn2 = nn.BatchNorm2d(128)
        ...
        self.ex2linear = make_classifier(self.image_size, self.out_dim) 


        ex3conv1 = nn.Conv2d(64, 128, kernel_size=3, stride=2, padding=1, bias=False)
        ex3bn1 = nn.BatchNorm2d(128)
        ...
        self.ex3linear = make_classifier(self.image_size, self.out_dim) 

        self.ex4linear = make_classifier(self.image_size, self.out_dim) 
...
class VGG11MCEarlyExit(VGG11EarlyExit):
    def __init__(self,dropout_exit = False, dropout = None, dropout_p = 0.5, n_exits = 4, out_dim = 100, *args,  **kwargs):
        ...
        if self.dropout_exit:
            ...
            self.ex1linear = make_classifier(self.image_size,self.out_dim,self.dropout_p)
            self.ex2linear = make_classifier(self.image_size,self.out_dim,self.dropout_p)
            self.ex3linear = make_classifier(self.image_size,self.out_dim,self.dropout_p)
            self.ex4linear = make_classifier(self.image_size,self.out_dim,self.dropout_p)
